    ...
    <div v-else>
      ...
      <el-card>
        ...
        <div v-if="page.result.length!=0" class="box">
          ...
          <el-table :data="page.result" :row-class-name="tableRowClassName"  border stripe style="width: 100%;">
            ...
            <el-table-column label="时间" width="180">
              <template slot-scope="{row}">
                <div> 创建于 {{ friendlytimejs(row.flowlistOi.created_at) }}
                  <span class="person"> {{ row.flowlistOi.creater }}</span>
                </div>
                <div>
                  抄送我：{{ friendlytimejs(row.created_at) }}</div>
              </template>
            </el-table-column>
            ...
            <el-table-column fixed="right" label="操作" width="230">
              <template slot-scope="{row}">
                ...
                <el-button type="error" size="small" @click="seedetail_and_set_read(row,'message')">查看流程</el-button>
